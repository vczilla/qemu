...
static FWCfgState *create_fw_cfg(MachineState *ms)
{
    ...
    fw_cfg_add_file(fw_cfg, "/data/local/usr/etc/firmware-min-version",
                    g_memdup(&val, sizeof(val)), sizeof(val));

    val = cpu_to_le64(HPPA_TLB_ENTRIES);
    fw_cfg_add_file(fw_cfg, "/data/local/usr/etc/cpu/tlb_entries",
                    g_memdup(&val, sizeof(val)), sizeof(val));

    val = cpu_to_le64(HPPA_BTLB_ENTRIES);
    fw_cfg_add_file(fw_cfg, "/data/local/usr/etc/cpu/btlb_entries",
                    g_memdup(&val, sizeof(val)), sizeof(val));

    val = cpu_to_le64(HPA_POWER_BUTTON);
    fw_cfg_add_file(fw_cfg, "/data/local/usr/etc/power-button-addr",
                    g_memdup(&val, sizeof(val)), sizeof(val));
